...
@pytest.mark.parametrize('max_syllables', [None, 10, 100])
def test_prep(dir_path, max_wavs, max_syllables, kwargify):
    kwargs = kwargify(dir_path=dir_path, max_wavs=max_wavs, max_syllables=max_syllables)
    ...
    if max_syllables:
        assert segedpsds.shape[0] <= max_syllables


@pytest.mark.smoke
@pytest.mark.parametrize(
    'dir_path, output_dir_path, max_wavs, max_syllables',
    [
        # default max_wavs, max_syllables
        ...
        # default max_wavs, but specify max_syllables
        ...
        # specify max_wavs and max_syllables
...
def test_prep_and_save(dir_path, output_dir_path, max_wavs, max_syllables, tmp_path, kwargify):
    ...
    kwargs = kwargify(dir_path=dir_path, output_dir_path=output_dir_path,
                      max_wavs=max_wavs, max_syllables=max_syllables)
    ...
    for a_dir_path, an_output_dir_path in zip(dir_path, output_dir_path):
        ...
        if max_syllables:
            assert saved.shape[0] <= max_syllables
