...
logger = logging.getLogger('songdkl')  # 'base' logger


def config_logging_for_cli(log_dst: str | None = None,
    ...
    for handler in (file_handler, rich_handler):
        if handler:
...
def log_version(logger: logging.Logger) -> None:
    logger.log(
        msg=f"songdkl version: {__version__}",
        level=logging.INFO
    )
