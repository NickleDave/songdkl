...
def get():
    ...
    prep_subparser.add_argument('--max-syllables', type=str, default=10000,
                                help=('Maximum number of segmented syllables per directory to use when generating '
                                      'power spectral densities (PSDs). Default is 10000.'))
    prep_subparser.add_argument('--psds-per-syl', type=str, default=1,
                                help='Number of PSDs to compute per syllable. Default is 1.')

    # NOTE loop below that adds common arguments to both the `calculate` and `numsyls` subparsers
    # ---- calculate command ----
    ...
    for subparser in (calculate_subparser, numsyls_subparser):
        # add the arguments passed to prep
        subparser.add_argument('--max-wavs', type=int, default=120,
                               help='Maximum number of .wav files to use. Default  is 120.')
        subparser.add_argument('--max-syllables', type=int, default=10000,
                               help=('Maximum number of segmented syllables per directory to use when generating '
                                     'power spectral densities (PSDs). Default is 10000.'))
        subparser.add_argument('--psds-per-syl', type=int, default=1,
                               help='Number of PSDs to compute per syllable. Default is 1.')
        subparser.add_argument('--n-basis', type=int, default=50,
                               help='Number of PSDs to use for the basis set. Default is 50.')
        subparser.add_argument('--basis', type=str, default='first', choices={'first', 'random'},
